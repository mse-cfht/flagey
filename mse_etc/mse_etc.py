...
class MseSpectrum:
    ...
    def apply_throughput_front(self):
        ...
        struc = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_struc.dat', format='ascii')
        if self.coating == 'ZeCoat':
            m1 = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_m1_zecoat.dat', format='ascii')
        else:
            m1 = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_m1_gemini.dat', format='ascii')
        pfue = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_pfue_WS.dat', format='ascii')
        pfhs = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_pfhs.dat', format='ascii')
        # Interpolate
    ...
    def apply_injeff(self):
        ...
        ie_for_etc = np.load(here_path + '/THROUGHPUT/no_segments_injeff_curve_for_etc_'
        ...
        iqs = sorted(list(ie_for_etc.keys()))
        iqvals = [float(iqs[i][2:]) for i in range(len(iqs))]
        # isolate IE curves of interest
        ie_etc = {}
        for iq in iqs:
            ie_etc[iq] = ie_for_etc[iq][zd_str][fiber]

        # Interpolate at desired IQ
        ie_coarse = [np.interp(self.seeing, iqvals, [ie_etc[iq][w] for iq in iqs]) for w in range(len(ie_wav))]
    ...
    def apply_throughput_back(self):
        ...
        poss = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_poss.dat', format='ascii')
        if self.spectro == 'LR' or self.spectro == 'MR':
            fiber = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_fits_lmr.dat', format='ascii')
            if self.spectro == 'LR':
                spectro = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_spec_lr.dat', format='ascii')

                # MOONS-like version
                if self.lmr == 'moons':
                    spectro_j = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_spec_mr_moons_J.dat', format='ascii')
                    spectro_h = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_spec_mr_moons_H.dat', format='ascii')
                ...
                spectro = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_spec_mr.dat', format='ascii')

        elif self.spectro == 'HR':
            spectro = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_spec_hr.dat', format='ascii')
            fiber = Table.read(here_path + '/THROUGHPUT/mse_etc_throughput_fits_hr.dat', format='ascii')
